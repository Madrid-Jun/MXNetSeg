...
class BDDSegmentation(SegmentationDataset):
    """
    BDD100k for semantic segmentation
    Reference: [1] F. Yu et al., “BDD100K: A Diverse Driving Dataset for
    ...
    def __init__(self, root=os.path.expanduser('~/.mxnet/datasets/bdd'), split='train',
                 mode=None, transform=None, **kwargs):
        super(BDDSegmentation, self).__init__(root, split, mode, transform, **kwargs)
        self.images, self.masks = _get_bdd_pairs(os.path.join(self.root, self.BASE_DIR), self.split)
