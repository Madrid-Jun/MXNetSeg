...
class LadderDenseNet(SegBaseDenseNet):
    ...
    def __init__(self, nclass, backbone='densnet169', aux=True, height=None, width=None,
        ...
        with self.name_scope():
            ...
            if self.aux:
                self.auxlayer = FCNHead(nclass, in_channels=decoder_capacity, norm_layer=norm_layer,
                                        norm_kwargs=norm_kwargs)
...
class _LadderHead(nn.HybridBlock):
    ...
    def __init__(self, nclass, decoder_capacity, input_height, input_width,
        ...
        with self.name_scope():
            ...
            self.fusion_c3 = LateralFusion(decoder_capacity, input_height // 16, input_width // 16,
                                           norm_layer=norm_layer, norm_kwargs=norm_kwargs)
            self.fusion_c2 = LateralFusion(decoder_capacity, input_height // 8, input_width // 8,
                                           norm_layer=norm_layer, norm_kwargs=norm_kwargs)
            self.fusion_c1 = LateralFusion(decoder_capacity, input_height // 4, input_width // 4,
                                           norm_layer=norm_layer, norm_kwargs=norm_kwargs)
